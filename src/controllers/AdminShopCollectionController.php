...
class AdminShopCollectionController extends BackendModelStandartController
{
    public function init()
    {
        ...
        $this->modelHeader = function () {
            return $this->renderPartial("@skeeks/cms/shop/views/admin-shop-collection/_model_header", [
                'model' => $this->model,
            ]);
        };
                    ...
                    'custom' => [
                        ...
                        'value'     => function (ShopCollection $model) {
                            ...
                            if ($model->sx_id) {
                                ...
                                $apiUrl = isset(\Yii::$app->skeeksSuppliersApi) ? \Yii::$app->skeeksSuppliersApi->getCollectionUrl($model->sx_id) : "#";
                            ...
                            $media = BackendEntityLink::widget([
                                'controllerId' => '/shop/admin-shop-collection',
                                'modelId'      => $model->id,
                ...
                'fields' => [
                    'sx_info_lock_notice' => [
                        ...
                        'content' => $isSxInfoLocked ? \yii\bootstrap\Alert::widget([
                            ...
                            'body'        => 'Коллекция связана с сервисом SkeekS Товары и синхронизация информации включена. Поля, которые обновляются из сервиса, закрыты от ручного редактирования, чтобы изменения не перезатирались. Чтобы изменить эти данные вручную, отключите синхронизацию в карточке коллекции.',
                        ]) : '',
                    ],
    ...
    protected function _isSxInfoUpdateLocked(ShopCollection $model = null)
    {
    ...
    protected function _restoreSxInfoLockedAttributes($event, array $attributes)
    {
        ...
        foreach ($attributes as $attribute) {
            if ($model->hasAttribute($attribute)) {
                ...
                if ($freshModel === null) {
                    $freshModel = ShopCollection::findOne($model->id);
                }
                if ($freshModel && $freshModel->canGetProperty($attribute)) {
                    $model->{$attribute} = $freshModel->{$attribute};
                }
            }
        }
    }

    protected function _registerSxInfoLockedFieldsAssets(ShopCollection $model, array $attributes)
    {
        $inputNames = [];
        foreach ($attributes as $attribute) {
