...
new yii\console\Application(['id'=>'offer-client-test','basePath'=>__DIR__,'vendorPath'=>'/deps','extensions'=>[],
    'components'=>['db'=>['class'=>yii\db\Connection::class,'dsn'=>'mysql:host=gpd-receiver-db','username'=>'root']]]);
$db=Yii::$app->db;$name='offer_client_'.bin2hex(random_bytes(5));$db->createCommand("CREATE DATABASE $name")->execute();$db->createCommand("USE $name")->execute();
foreach(['m260917_030000_gpd_catalog_receiver','m260917_120000_gpd_catalog_application','m260918_010000_gpd_offer_receiver'] as $class)require dirname(__DIR__).'/src/migrations/'.$class.'.php';
...
class OfferTestTransport implements CatalogTransportInterface {
    public $pages=[];
    public function request(string $method,string $endpoint,array $data=[]):array {
        if(!$this->pages)throw new RuntimeException('Unexpected request');return array_shift($this->pages);
    }
}
class OfferTestWriter implements CatalogWriterInterface {
    public $fail=false;
    public function prepare(array $item):void{}
    public function apply(array $item,array $state):array {
        Yii::$app->db->createCommand()->update('{{%shop_gpd_offer_state}}',['local_product_id'=>7],['connection_id'=>'same','product_id'=>$item['id']])->execute();
...
try {
 foreach(['','sx_'] as $prefix) {
    ...
    (new m260917_030000_gpd_catalog_receiver())->safeUp();(new m260917_120000_gpd_catalog_application())->safeUp();(new m260918_010000_gpd_offer_receiver())->up();
    $wire=new OfferTestTransport();$catalog=new CatalogReceiver($db,'same',$wire);$catalog->register(1,'https://fixture.invalid/v2',str_repeat('a',64));
    $receiver=new CatalogReceiver($db,'same',$wire,'offers');$receiver->register(1,'https://fixture.invalid/v2',str_repeat('a',64));
    $wire->pages=[['protocol'=>1,'stream'=>'offers','seeded'=>true],['total'=>1,'cursor'=>'r1','changes_cursor'=>'c1']];$receiver->receive();
    check($catalog->state()['phase']==='bootstrap','offer bootstrap does not alter catalog state');
    ...
    check($receiver->state()['phase']==='changes','offer bootstrap completed');
    check((int)$db->createCommand('SELECT COUNT(*) FROM {{%shop_gpd_catalog_state}}')->queryScalar()===0,'offer receipts do not pollute product state');
    $item['data']=['id'=>5,'kind'=>'brands','source_id'=>7,'payload'=>['id'=>7,'name'=>'Fixture']];$wire->pages=[['items'=>[$item]]];
    $writer=new OfferTestWriter();$applier=new CatalogApplier($db,'same',$wire,$writer,'offers');$entry=$applier->page()[0];
    ...
    $row=$db->createCommand('SELECT * FROM {{%shop_gpd_offer_state}}')->queryOne();check($row['local_product_id']===null&&(int)$row['applied_revision']===0,'failed application rolls back identity and ACK');
    ...
    $receiver->receive();check($applier->remaining()===1,'new offer revision remains applicable');
    check($catalog->state()['cursor']===null,'catalog cursor remains untouched');
 }
 echo "PASS $checks offer receiver/application checks with two prefixes.\n";
